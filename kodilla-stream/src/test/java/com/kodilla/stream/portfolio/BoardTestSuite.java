...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        //When
        List<TaskList> averageTimeTaskList = new ArrayList<>();
        averageTimeTaskList.add(new TaskList("In progress"));
        double averageTime = project.getTaskLists().stream()
                .filter(averageTimeTaskList::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .mapToDouble(i -> i.until(LocalDate.now(),ChronoUnit.DAYS)).average().getAsDouble();

        //Then
        Assertions.assertEquals(averageTime,10);
    }
}